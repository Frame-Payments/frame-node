...
- **Removed `sdk.sonarSessions` and `sdk.onboarding` — dead surface, never a real backend route (FRA-6648).** Both hit paths with no corresponding endpoint in Frame's iOS SDK or backend: `sonarSessions` posted to `/v1/sonar_sessions` (the real, working resource is `chargeSessions`, `/v1/charge_sessions` — unaffected); `onboarding` posted to `/v1/onboarding/sessions` (note the slash; the real resource is `onboardingSessions`, `/v1/onboarding_sessions`, with underscore — unaffected). Neither was ever `@deprecated`-tagged, and grepping frame-ios's full Networking layer turns up no trace of either path — this reads as a naming mistake made once and never caught, in the same vein as the `Account.payout_payment_method_id` field removed in 2.1.5. **Breaking change** for any caller using `sdk.sonarSessions.*` or `sdk.onboarding.*` — those calls were already failing against the real API (404) and had no working replacement path; switch to `sdk.chargeSessions` and `sdk.onboardingSessions` respectively.
- **`ChargeSession.id` is now optional; `sonar_session_id` added (FRA-6648).** The server sends the id as `sonar_session_id`, not `id` — confirmed against Frame-iOS's wire contract. `id` is kept as an optional aliased field for existing callers reading it, but code that assumed `id` is always present should switch to `sonar_session_id`.
- **`CreateChargeSessionParams` / `UpdateChargeSessionParams` gain `fingerprint_visitor_id`, `account_id`, and `sealed_result` (FRA-6648).** All optional, matching Frame-iOS. Omit a field entirely (rather than passing it as `undefined`) to leave it off the wire — the API treats an explicit `null` differently from an absent field.
- **New `idv` namespace (FRA-6648).** `idv.createSession()` / `idv.completeSession(inquiryId)` — typed support for `POST /v1/idv/session` and `POST /v1/idv/complete`.
- **`accounts.electPayoutMethod(id, paymentMethodId)` (FRA-6648).** `POST /v1/accounts/{id}/elect_payout_method` — makes an existing ACH payment method the account's payout destination.
- **`configuration.getAllConfiguration()` (FRA-6648).** Aggregates the `evervault`/`fingerprint`/`sift`/`legal`/`mapbox` config blocks in one round-trip via `GET /v1/config/all`, matching Frame-iOS's FRA-6251 aggregate. Also adds standalone `getFingerprintConfiguration()`, `getLegalConfiguration()`, and `getMapboxConfiguration()`.
- **`refunds.cancel(id)` (FRA-6648).** `POST /v1/refunds/{id}/cancel` — cancels a pending refund. Server-only; call with your secret key, not from a client app.
